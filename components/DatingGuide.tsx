...
export default function DatingGuide({ guide }: { guide: DatingGuideData }) {
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: safeJson({
        "@context": "https://schema.org", "@type": "Article",
        headline: `${guide.name} : avis et accès au Québec`,
        datePublished: "2026-09-07", dateModified: "2026-09-07", inLanguage: "fr-CA",
      ...
      <section className="mb-10 grid gap-5 md:grid-cols-4">
        {[["Note éditoriale", guide.rating], ["Meilleur pour", guide.bestFor], ["Communication", guide.communication], ["Inscription", guide.signup]].map(([label, value]) => (
          <div key={label} className="rounded-3xl border border-zinc-800 bg-zinc-950 p-6">
            <p className="text-sm font-bold text-pink-400">{label}</p><p className="mt-3 text-xl font-black">{value}</p>
          </div>
        ))}
      </section>
      ...
      <section className="mb-10 rounded-3xl border border-pink-900/60 bg-gradient-to-br from-zinc-950 to-pink-950/30 p-8">
        ...
        <div className="mt-7 flex flex-wrap gap-3">
          <a href={guide.affiliate} target="_blank" rel="nofollow sponsored noopener noreferrer" className="rounded-xl bg-pink-600 px-6 py-3 font-bold hover:bg-pink-500">Visiter {guide.name} →</a>
          <Link href="/comparatifs/top-rencontres-adultes-quebec" className="rounded-xl border border-pink-500 px-6 py-3 font-bold text-pink-400 hover:bg-pink-500/10">Comparer les 5 plateformes</Link>
        </div>
      </section>
      ...
      <section className="mb-10 rounded-3xl border border-zinc-800 bg-zinc-950 p-8">
        ...
        <div className="mt-7 space-y-7">{guide.faqs.map((faq) => <div key={faq.question}><h3 className="text-xl font-bold">{faq.question}</h3><p className="mt-2 leading-7 text-gray-300">{faq.answer}</p></div>)}</div>
      </section>
    </>
  );
}
